...
#     color = 'black' if row['cluster'] == -1 else colors[row['cluster'] % len(colors)]
#     folium.CircleMarker(
...
# map_clusters.save('dbscanfebwjitter.html')
...
for _, row in gdf.iterrows():
    ...
    folium.CircleMarker(
        ...
        fill=True,
        fill_color=color,
        popup=popup_text  # Add popup with disease information
    ).add_to(map_clusters)
